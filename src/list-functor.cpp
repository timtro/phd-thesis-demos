...
// The following tests are not completely orthogonal. Each of the
// appropriate standard container types for which I have defined
// functors have their functor interface tested here.
...
TEST_CASE("Given a std::vector<A> …", "[fmap]") {
  ...
  SECTION("… and a function f : A → B, fmap should produce a "
          "std::vector<B>:") {
    auto bs = fmap(f, as);
  ...
  SECTION(
      "… fmap given only a function f : A → B, should produce a "
      "function "
      "object, which when subsequently a std::vector<A> will "
      "bind "
      "appropriately to an uncurried fmap variant.") {
    auto lifted_f = fmap(f);
    REQUIRE((std::is_same<decltype(lifted_f(as))::value_type,
        B>::value));
    REQUIRE(lifted_f(as).at(0) == B{});
  }

  SECTION("… mapping over it with the id-function should leave "
          "it unchanged") {
...
TEST_CASE(
    "For std::vector, calling fmap should induce the same "
    "constructor "
  ...
  rb.reserve(ra.size()); // If no reserve, you get extra copy
                         // ctrn!
  std::transform(
      ra.cbegin(), ra.cend(), std::back_inserter(rb), id);
...
TEST_CASE(
    "For std::list, calling fmap should induce the same "
    "constructor and "
...
TEST_CASE(
    "The std::array typeconstructor will not be a regular "
    "functor,  "
    "because it can not be constructed with the form F<A>. For "
    "integer "
    "dependant type constructors, is_functor_N should test "
    "true:") {
...
TEST_CASE("Given a std::array<A,·> …", "[fmap]") {
  ...
  SECTION(
      "… fmap given only a function f : A → B, should produce a "
      "function "
      "object, which when subsequently a std::array<A> will "
      "bind "
      "appropriately to an uncurried fmap variant.") {
    auto lifted_f = fmap(f);
    REQUIRE((std::is_same<decltype(lifted_f(as))::value_type,
...
TEST_CASE(
    "For std::array, calling fmap should induce the same "
    "constructor and "
    "assignment calls as simply using std::transform, plus a "
    "move "
    "construction:",
    "[CtorAs], [fmap]") {
  // NB: Extra move construction is because moving a std::array
  // calls move on each element. So, a move on std::array is O(N)
  // instead of O(1) like the other containers.
...
TEST_CASE("Given a std::deque<A> …", "[fmap]") {
  ...
  SECTION(
      "… fmap given only a function f : A → B, should produce a "
      "function "
      "object, which when subsequently a std::deque<A> will "
      "bind "
      "appropriately to an uncurried fmap variant.") {
    auto lifted_f = fmap(f);
    REQUIRE((std::is_same<decltype(lifted_f(as))::value_type,
...
TEST_CASE(
    "For std::deque, calling fmap should induce the same "
    "constructor "
    "and assignment calls as simply using std::transform.",
    "[CtorAs], [fmap]") {
